...
static off_t _db_readptr(DB *db, off_t offset) {
  ...
  if (read(db->idxfd, ascliptr, PTR_SZ) != PTR_SZ)
...
static off_t _db_readidx(DB *db, off_t offset) {
  ...
  ascillen[IDXLEN_SZ] = 0;
  if ((db->idxlen = atoi(ascillen)) < IDXLEN_MIN || db->idxlen > IDXLEN_MAX)
    err_dump("_db_readidx: invaild length");
...
static void _db_writeidx(DB *db, const char *key, off_t offset, int whence,
  ...
  if (whence == SEEK_END)
    if (un_lock(db->idxfd, ((db->nhash + 1) * PTR_SZ) + 1, SEEK_END, 0) < 0)
